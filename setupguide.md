...
To proceed with the next steps, you may need to open another terminal or tmux window. If in a new terminal, connect to your server again. If using tmux, create a new window with `Ctrl+b` then `c`.

Retrieve your node peer ID in the new session:
...
Reattach to your tmux session and restart your node to apply changes and start earning $QUIL:
...
Detach from the tmux session when you wish to leave the process running in the background.

## 12. Check Your $QUIL Balance

After letting your node run for some time, check your $QUIL balance by reattaching to the tmux session:
